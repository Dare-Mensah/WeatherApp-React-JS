...
function WeatherIndex5() {

    const [weatherData, setWeatherData] = useState()

    useEffect(() => {
        const API = 'https://api.open-meteo.com/v1/forecast?latitude=35.69&longitude=139.69&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&current_weather=true&timeformat=unixtime&timezone=auto'
        fetch(API)
        .then(res => res.json())
        .then(data => {
    ...
    <Navbar/> 
        <div className='container'>
            <div className='top'>
                ...
                <h3 className='location'>Tokyo</h3>
